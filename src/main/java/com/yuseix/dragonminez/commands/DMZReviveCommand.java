	...
	private static int revive(Collection<ServerPlayer> players, CommandSourceStack source) {
		for (ServerPlayer player : players) {
			DMZStatsProvider.getCap(DMZStatsCapabilities.INSTANCE, player).ifPresent(cap -> {
				...
				player.sendSystemMessage(Component.translatable("commands.dmzrevive.revived", source));
				source.sendSystemMessage(Component.translatable("commands.dmzrevive.success", player));
			});
		}
